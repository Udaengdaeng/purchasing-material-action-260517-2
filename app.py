...
def build_analysis(
    ...
    summary_cols = [
        "prod_key", "buyer_po", "buyer", "style", "style_color", "prod_qty",
        "need_date", "sewing_to", "ship_date", "material_status", "remark",
        "shortage_items", "urgent_items", "risk", "main_action"
    ]
    ...
    for _, p in prod_target.iterrows():
        ...
        for _, b in matched.iterrows():
            ...
            # 핵심 수정:
            # 1) MATERIAL OK면 부족 판단하지 않음
            # 2) MATERIAL에 TBA/ETA/지연/부족 등 위험 단어가 있으면 위험 우선
            # 3) 그 외에는 재고+입고-출고 참고 가용량으로 부족 참고
            if is_material_ok(p["material_status"]):
                ...
                if shortage <= 0 and not risky_by_status:
                    risk = "정상"
                    basis = "참고 가용량 기준 커버 가능"
                    action = "재고/입고 흐름 기준으로 커버 가능"
                elif risky_by_status or (days_left is not None and days_left < lead):
                    risk = "긴급"
                    basis = "MATERIAL/REMARK 위험 표시 또는 리드타임 부족"
                    if rec > 0:
                        action = f"{b['supplier'] or '공급업체 확인'}에 {b['item']} {fmt_qty(rec)}{b['unit']}를 생산 전까지 입고 가능한지 즉시 확인"
                    else:
                        action = f"{b['supplier'] or '공급업체 확인'}에 {b['item']} 입고/ETA 상태 즉시 확인"
                else:
                    risk = "주의"
                    basis = "참고 가용량 기준 부족 가능"
...
def show_dashboard(summary: pd.DataFrame, detail: pd.DataFrame):
    ...
    st.dataframe(
        view[[
            ...
            "생산시작", "생산종료", "material_status", "shortage_items", "urgent_items", "main_action"
        ]].rename(columns={
            "buyer_po": "PO No.",
            "style": "Style",
            "style_color": "Color",
